...
from taz_rl.rlenv.local_taz_env_v8 import SumoTazEnvV8
...
ROUTE_CACHE_ROOT = os.path.join(SUMO_PATH, "routes_v8_deterministic")
LEGACY_ROUTE_CACHE_ROOTS = [os.path.join(SUMO_PATH, "routes_v7_deterministic")]
...
ENV_V8_CONFIG = dict(
    ...
    max_green=120,
    reward_clip=2.0,
    waiting_reward_weight=0.45,
    emission_reward_weight=0.45,
...
# Checkpoints
LOAD_MODEL = False
CHECKPOINT_PATH = os.path.join(SCRIPT_DIR, "checkpoints_v8", "checkpoint_ppo_v8_final.pt")
...
def main():
    ...
    log_file = os.path.join(sumo_standalone_dir, "command_log_v8.txt")
    ...
    env = SumoTazEnvV8(sumoSimulator=sumo, stepSize=300, **ENV_V8_CONFIG)
    ...
    policy = ActorCriticV8(obs_dim, act_dim, action_limit=ACTION_LIMIT)
    policy.train()
    ...
    csv_path = os.path.join(SCRIPT_DIR, "training_history_v8.csv")
    json_path = os.path.join(SCRIPT_DIR, "training_history_v8.json")
    ckpt_dir = os.path.join(SCRIPT_DIR, "checkpoints_v8")
    detail_dir = os.path.join(SCRIPT_DIR, "training_details_v8")
    ...
    print(f"[INFO] v8 TAZ count: {len(taz_ids)} | IDs: {taz_ids}")
    print(f"[INFO] v8 TLS count: {len(tls_ids)}")
    ...
    for episode_idx in range(start_episode, n_episodes):
        ...
        route_folder_path, should_generate_routes = _resolve_route_folder(simulation_date, timeslot_clean, total_cars_random)
        ...
        while True:
            ...
            try:
                step_td = env.step(TensorDict({"action": a}, batch_size=[]))
        ...
        stats = ppo_update(
            policy, optim, batch_obs, batch_action_mask, batch_act, batch_logp, batch_adv, batch_ret,
        ...
        episode_reward_by_taz = rewards_t.sum(dim=0).cpu()
        reward_mean = float(episode_reward_by_taz.mean().item())
        ...
        reward_by_taz = {
            taz: float(episode_reward_by_taz[idx].item())
        ...
        row = {
            ...
            "num_steps": int(len(rewards_list)),
            "reward_basis": str(reward_components.get("reward_basis", "per_taz_step_delta")),
            "episode_reward_mean": reward_mean,
            "episode_reward_std": float(episode_reward_by_taz.std(unbiased=False).item()),
            "episode_reward_min": float(episode_reward_by_taz.min().item()),
            "episode_reward_max": float(episode_reward_by_taz.max().item()),
            "episode_reward_sum": float(episode_reward_by_taz.sum().item()),
        ...
        torch.save(ckpt_payload, os.path.join(ckpt_dir, f"checkpoint_ppo_v8_episode{episode_idx}.pt"))
        ...
        if reward_mean > best_reward:
            ...
            torch.save(best_payload, os.path.join(ckpt_dir, "checkpoint_ppo_v8_best.pt"))
    ...
    torch.save(
        ...
        os.path.join(ckpt_dir, "checkpoint_ppo_v8_final.pt"),
    )
